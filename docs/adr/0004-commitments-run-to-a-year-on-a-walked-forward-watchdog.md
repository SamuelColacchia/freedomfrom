...
**In practice the first one usually gets there first, and that is worth saying rather than hiding.** Across two device runs the `Monitor` released 1 and 2 seconds past the deadline, with the app closed and nobody touching the phone — fast enough that a person trying to beat it to the target could not, which is why S2 had to disarm the final window to be reachable at all. So the third point is insurance rather than the mechanism, and on the evidence so far it is insurance against something that has not happened yet. It is kept because `DeviceActivity` callbacks are documented as firing when the device is *next in use* rather than on time, and because the same capability is what makes the self-shield fix legal (ADR 0005) — that one has no redundant path, since no other process can recognise freedomfrom's own token.
...
- **The `ShieldConfig` extension is no longer purely presentational.** It reads the keychain record and may mutate a `ManagedSettingsStore`, inside a memory-starved process. **Unverified**, and now a first-order hardware smoke check. If it proves impossible, the deferred notification is the fallback and this ADR is amended rather than reopened. **Hardware check S2 came back green**, so the third row of the table above stands and the notification stays in reserve: on an iPhone 14 Pro Max on iOS 26.6.1, with the final window deliberately unregistered so nothing else could take the release, the extension woke, read the mirrored deadline, cleared the store and logged the mutation 32ms later, without being killed. Reading the record is S1's story and went the other way — it reaches the deadline through the App Group, not the Keychain (ADR 0002).
