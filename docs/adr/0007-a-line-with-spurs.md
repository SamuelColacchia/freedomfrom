...
- **What Targets remembers between commitments is now a decision with nowhere to hide.** Opening blank makes every commitment a fresh deliberate choice; remembering the last selection makes re-committing one tap. **Answered by [ADR 0008](./0008-the-root-holds-a-draft.md)**: it remembers, as a draft covering targets and length, which only the user and a clean slate edit.
